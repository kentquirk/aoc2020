...
func (p *AoCCalc) Init() {
	...
	_rules = [...]func() bool{
		...
		/* 3 e3 <- <((minus value Action2) / value)> */
		func() bool {
		...
		/* 4 value <- <((<[0-9]+> sp Action3) / (open e1 close))> */
		func() bool {
		...
		/* 12 Action0 <- <{ p.AddOperator(TypeMultiply) }> */
		nil,
		/* 13 Action1 <- <{ p.AddOperator(TypeAdd) }> */
		nil,
		/* 14 Action2 <- <{ p.AddOperator(TypeNegation) }> */
		nil,
		nil,
		/* 16 Action3 <- <{ p.AddValue(buffer[begin:end]) }> */
		nil,
	}
	p.rules = _rules
}
